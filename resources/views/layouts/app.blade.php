...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-screen flex flex-col antialiased selection:bg-indigo-500 selection:text-white transition-colors duration-200">
    ...
    <!-- Floating Glass Sticky Navigation Header -->
    <header class="sticky top-0 z-50 px-4 sm:px-6 py-3 transition-all duration-300">
        <nav class="max-w-7xl mx-auto glass-nav rounded-2xl border border-white/60 dark:border-slate-800 shadow-lg shadow-slate-200/50 dark:shadow-slate-950/50 px-5 py-2.5 flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-6 font-bold text-xs sm:text-sm text-slate-600 dark:text-slate-300">
                <a href="{{ route('home') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors {{ Request::is('/') ? 'text-indigo-600 dark:text-indigo-400 font-extrabold' : '' }}">Beranda</a>
                <a href="{{ route('katalog') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors {{ Request::is('katalog') ? 'text-indigo-600 dark:text-indigo-400 font-extrabold' : '' }}">Jelajahi</a>
                <a href="/#kategori" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">Kategori</a>
                <a href="{{ route('organizer.register') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors {{ Request::is('organizer/register') ? 'text-indigo-600 dark:text-indigo-400 font-extrabold' : '' }}">Mitra Organizer</a>
                <a href="{{ route('tentang') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors {{ Request::is('tentang') ? 'text-indigo-600 dark:text-indigo-400 font-extrabold' : '' }}">Tentang Kami</a>
                <a href="{{ route('bantuan') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors {{ Request::is('bantuan') ? 'text-indigo-600 dark:text-indigo-400 font-extrabold' : '' }}">Bantuan</a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-3">
                ...
                @auth
                    ...
                    <!-- User Profile Pill -->
                    <a href="{{ route('profil') }}" class="flex items-center gap-2 px-3 py-1.5 bg-slate-100/90 dark:bg-slate-800/90 hover:bg-indigo-50 dark:hover:bg-indigo-950 rounded-xl border border-slate-200/60 dark:border-slate-700 transition">
                        @if(Auth::user()->avatar)
                        ...
                        @endif
                        <span class="text-xs font-extrabold text-slate-800 dark:text-slate-100 max-w-[110px] truncate">{{ Auth::user()->name }}</span>
                    </a>
                ...
                @else
                    <a href="{{ route('login') }}" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black text-xs shadow-md shadow-indigo-200 dark:shadow-none hover:-translate-y-0.5 transition-all flex items-center gap-1.5">
        ...
        <div id="mobileMenuDrawer" class="hidden md:hidden mt-2 glass-nav rounded-2xl p-5 shadow-2xl border border-white/40 dark:border-slate-800 space-y-4">
            <nav class="flex flex-col space-y-3 font-bold text-slate-700 dark:text-slate-200 text-sm">
                <a href="{{ route('home') }}" class="px-3 py-2 hover:bg-indigo-50 dark:hover:bg-slate-800 hover:text-indigo-600 rounded-xl transition {{ Request::is('/') ? 'text-indigo-600 bg-indigo-50 dark:bg-slate-800' : '' }}">Beranda</a>
                <a href="{{ route('katalog') }}" class="px-3 py-2 hover:bg-indigo-50 dark:hover:bg-slate-800 hover:text-indigo-600 rounded-xl transition">Jelajahi Event</a>
            ...
            <div class="pt-4 border-t border-slate-200/80 dark:border-slate-800">
                @auth
                    <div class="space-y-3">
                        <a href="{{ route('profil') }}" class="flex items-center gap-3 px-3 py-2 bg-slate-100/80 dark:bg-slate-800 rounded-xl">
                            @if(Auth::user()->avatar)
                ...
                @else
                    <a href="{{ route('login') }}" class="block w-full text-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black text-xs shadow-lg shadow-indigo-200 dark:shadow-none transition">
                        Masuk / Daftar Akun
                    </a>
                @endauth
            </div>
    ...
    <!-- Dark Footer Design (Identical to Image Mockup) -->
    <footer class="bg-slate-950 text-slate-400 pt-16 pb-12 px-4 sm:px-6 border-t border-slate-800/80">
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-12 pb-12 border-b border-slate-800/60">
            ...
            <div class="md:col-span-3 space-y-3">
                ...
                <ul class="space-y-2.5 text-xs sm:text-sm font-medium">
                    ...
                    <li><a href="{{ route('katalog') }}" class="hover:text-indigo-400 transition-colors">Jelajahi Event</a></li>
    ...
    <!-- Global Theme Switcher & Mobile Drawer JS -->
    <script>
