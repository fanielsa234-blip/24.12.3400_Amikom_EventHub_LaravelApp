...
<div class="max-w-xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">Edit Partner</h1>
            <p class="text-slate-500 dark:text-slate-400 text-xs mt-0.5">Perbarui informasi data partner <strong class="text-slate-800 dark:text-slate-200">{{ $partner->name }}</strong>.</p>
        </div>
    ...
    <!-- Form Card -->
    <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4 transition-colors">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $partner->name) }}" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition text-xs sm:text-sm font-medium text-slate-900 dark:text-white" required>
        </div>

        <div>
            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wider">Upload File Logo Baru (Opsional)</label>
            ...
            <p class="text-[11px] text-slate-400 dark:text-slate-400 mt-1 font-medium">*Unggah file baru dari laptop jika ingin mengganti logo saat ini (Max: 2MB).</p>
        </div>
        ...
        <div>
            ...
            <input type="url" name="logo_url" value="{{ old('logo_url', str_starts_with($partner->logo_url, 'storage/') ? '' : $partner->logo_url) }}" placeholder="https://example.com/logo.png" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition text-xs sm:text-sm font-medium text-slate-900 dark:text-white">
        </div>
        
        <div>
            <p class="text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wider">Preview Logo Saat Ini:</p>
            <div class="w-16 h-16 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden p-1">
                <img src="{{ str_starts_with($partner->logo_url, 'storage/') ? asset($partner->logo_url) : $partner->logo_url }}" class="w-full h-full object-contain" alt="{{ $partner->name }}" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($partner->name) }}&background=6366f1&color=fff&bold=true'">
            </div>
        </div>
