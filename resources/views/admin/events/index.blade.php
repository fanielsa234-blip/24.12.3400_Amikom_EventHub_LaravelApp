...
<div class="p-6">
    ...
    @if(session('success'))
        <div class="bg-green-100 text-green-700 p-4 rounded border border-green-200 mb-5">
            {{ session('success') }}
        </div>
    @endif
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td class="p-4 flex gap-2">
                        ...
                        <form action="{{ route('admin.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus data acara ini secara permanen?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-100 border border-red-200 px-3 py-1.5 rounded text-red-600 text-sm font-semibold hover:bg-red-600 hover:text-white transition">Hapus</button>
                        </form>
                    </td>
                </tr>
                @endforeach
